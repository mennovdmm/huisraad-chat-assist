...
import { ChevronRight, Mail, Bot, Trash2, LogOut } from "lucide-react";
...
const UserDashboard: React.FC = () => {
  useSEO({
    title: "User Dashboard - Huisraad Agents",
    description: "Start beschikbare agents in jouw Huisraad omgeving.",
      ...
      <header className="sticky top-0 bg-background/80 backdrop-blur p-4">
        <div className="flex items-center justify-between max-w-6xl mx-auto px-6 md:px-10">
          ...
          <div className="flex items-center gap-4">
            {/* User-only header: only user name and logout button */}
            <span id="userName" className="text-xs text-muted-foreground">
              {userName}
            </span>
            <button
              onClick={logout}
              className="flex items-center gap-1 px-1.5 py-0.5 rounded-md text-xs text-muted-foreground hover:text-foreground hover:bg-accent transition"
            >
